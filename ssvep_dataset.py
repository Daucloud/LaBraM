...
import scipy
import torch
from utils import get_ref_sig, filterbank, loadmat
import matplotlib.pyplot as plt
from plot_methods import plot_eeg
...
def readSubjectData(dataset_name, subject_name,
    ...
    if dataset_name == 'data':
        ...
        # SSVEP
        # 总共40个类，每个类重复6次，每个试次6s (其中前0.5s是刺激前数据，后5.5s是刺激后数据)
        ...
        if data_divide_method == 'block':
            ...
            if filterbank_num != 0:
                ...
                train_data = [trial[np.newaxis, :, :] for trial in train_data]
                test_data = [trial[np.newaxis, :, :] for trial in test_data]
...
def build_Benchmark_dataset_for_pretraining(root_dir, winLEN=1, remove_idx=1):
    ########### Benchmark ############
    dataset_name = 'Benchmark'
    # 使用除了S1之外的被试进行训练
    sub_name_list = ['S' + str(idx) for idx in range(1, 36)]
    # for remove_idx in [1, 2, 6, 8, 9, 10, 15, 17, 20, 23, 30, 35]:
    sub_name_list.remove('S' + str(remove_idx))
    n_fold = 6
...
def build_Benchmark_dataset_for_finetune(root_dir, winLEN=1, sub=1, test_block_idx=0):
    ########### Benchmark ############
    dataset_name = 'Benchmark'
    # 使用除了S1之外的被试进行训练
    sub_name = 'S' + str(sub)
...
def build_Benchmark_dataset_leave_one_out(root_dir, winLEN=1, test_sub=1, filterbank_num=1, latency=0.14):
    """
    Leave-one-subject-out Benchmark loader for 40-class classification.
    train: 34 subjects (all 6 blocks each)
    test: held-out subject (all 6 blocks)
    """
    srate = 250
    _CHANNELS = [
        'FP1', 'FPZ', 'FP2', 'AF3', 'AF4', 'F7', 'F5', 'F3', 'F1', 'FZ', 'F2',
    ...
    def _load_subject(subject_name):
        subject_file = root_dir + '/Benchmark/' + subject_name + '.mat'
        subject_data = scipy.io.loadmat(subject_file)
        subject_data = subject_data['data']  # (64, 1500, 40, 6)

        chnINX = [_CHANNELS.index(i) for i in ssvep_selected_channels]
        subject_data = subject_data[chnINX, round(srate * (0.5 + latency)):round(srate * 5.5), :, :]
        subject_label = np.repeat(np.arange(0, 40)[:, np.newaxis], repeats=6, axis=1)  # (40, 6)

        subject_data = subject_data.transpose(3, 2, 0, 1)  # (6, 40, ch, time)
        subject_label = subject_label.T  # (6, 40)
        ...
        subject_data = subject_data[:, :, :, :round(winLEN * srate)]
        subject_data = np.concatenate([block for block in subject_data], axis=0)  # (trials, ch, time)
        subject_label = subject_label.reshape(-1)

        trials = []
        for trial in subject_data:
            if filterbank_num != 0:
                trial = filterbank(trial, srate=srate, num_subbands=filterbank_num)
            else:
                trial = trial[np.newaxis, :, :]
            trial = np.transpose(trial, (1, 0, 2))  # (channels, subbands, time)
            trials.append(trial)

        trials = np.stack(trials, axis=0)
        labels = subject_label.astype(np.int64)
        return trials, labels

    all_subjects = ['S' + str(idx) for idx in range(1, 36)]
    test_name = 'S' + str(test_sub)
    train_names = [sub for sub in all_subjects if sub != test_name]

    train_trials = []
    train_labels = []
    for sub_name in train_names:
        trials, labels = _load_subject(sub_name)
        train_trials.append(trials)
        train_labels.append(labels)

    test_trials, test_labels = _load_subject(test_name)

    train_data = torch.from_numpy(np.concatenate(train_trials, axis=0)).float()
    train_targets = torch.from_numpy(np.concatenate(train_labels, axis=0)).long()
    test_data = torch.from_numpy(test_trials).float()
    test_targets = torch.from_numpy(test_labels).long()

    train_dataset = TensorDataset(train_data, train_targets)
    test_dataset = TensorDataset(test_data, test_targets)

    return train_dataset, test_dataset, ssvep_selected_channels
